...
from tensorflow.keras.layers import BatchNormalization, Conv2D, UpSampling2D, MaxPooling2D, Dropout, ConvLSTM2D
...
def get_unet_proper_lstm(filters, kernels, input_shape, activation='elu', dr=0):
    concat_axis = 3
    inputs = layers.Input(shape = (None, 32, 64, 2))

    clstm = ConvLSTM2D(filters=64, kernel_size=5, dilation_rate=2, padding='same', activation='tanh',
               data_format='channels_last')(inputs)

    bn0 = BatchNormalization(axis=3)(clstm)
    conv1 = Conv2D(32, (3, 3), activation='relu', padding='same', name='conv1_1')(bn0)
    bn1 = BatchNormalization(axis=3)(conv1)
    conv1 = Conv2D(32, (3, 3), activation='relu', padding='same')(bn1)
    bn2 = BatchNormalization(axis=3)(conv1)
